...
static bool riscv_iommu_have_msi_remap(struct riscv_iommu_device *iommu)
{
	return (iommu->caps & RISCV_IOMMU_CAPABILITIES_MSI_FLAT) && imsic_enabled();
}
				       ...
				       struct riscv_iommu_pc *pc)
{
	...
	if (FIELD_GET(RISCV_IOMMU_DC_IOHGATP_MODE, iohgatp) ==
		...
		/* else: IOTINVAL.VMA with GV=1, AV=PSCV=0, GSCID=DC.iohgatp.GSCID */
		riscv_iommu_cmd_send(iommu, &cmd);
...
static bool riscv_iommu_capable(struct device *dev, enum iommu_cap cap)
{
	...
	case IOMMU_CAP_GUEST_MSI_ISOLATION:
		return riscv_iommu_have_msi_remap(iommu);
	default:
		return false;
	}
}
