...
int kvm_riscv_vcpu_aia_imsic_update(struct kvm_vcpu *vcpu)
{
	...
	/*
	 * Update the IOMMU mapping outside vsfile_lock.  Holding vsfile_lock
	 * here would deadlock against kvm_irqfd_update(), which takes
	 * irqfds.lock then vsfile_lock(read), while this path takes
	 * vsfile_lock(write) then irqfds.lock inside kvm_riscv_vcpu_irq_update().
	 */
